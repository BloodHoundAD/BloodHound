...
function doQuery(query, start, end, preventCollapse) {
    ...
    sigma.neo4j.cypher({ url: localStorage.getItem("dbpath"), user: localStorage.getItem("uname"), password: localStorage.getItem("pwd") },
        query,
        sigmaInstance,
        function() {
            var startNode = null;
    ...
    if (node.data.node.type_user === true) {
        ...
        cancelQuery = $.ajax({
            ...
            success: function(json) {
                ...
                var nodeinfo = Mustache.render(compnodetemplate, {
                    label: node.data.node.label,
                    explicit_admins: c1,
                    sessions: c3,
                    unrolled_admin: c2,
                    first_degree_group: c4,
                    unrolled_group_membership: c5
                });
        ...
        cancelQuery = $.ajax({
            ...
            success: function(json) {
                ...
                var nodeinfo = Mustache.render(groupnodetemplate, {
                    label: node.data.node.label,
                    explicit_members: c1,
                    unrolled_members: c2,
                    adminto: c3,
    ...
    reader.onload = function(event) {
        ...
        ingestWorker.addEventListener('message', function(e) {
            ...
            if (e.data.message == 'baddata') {
                ...
                template = $('#ingestProgressBar').html();
                w = 0;
                h = Mustache.render(template, { "progress": 0, "total": e.data.len, "width": w });
                ...
                template = $('#ingestProgressBar').html();
                w = Math.floor((e.data.progress / e.data.len) * 100);
                h = Mustache.render(template, { "progress": e.data.progress, "total": e.data.len, "width": w });
                $('#uploadSelectDiv').html(h);
            } else if (e.data.message == 'commit') {
                getDBInfo();
            } else if (e.data.message == 'end') {
                getDBInfo();
                template = $('#ingestComplete').html();
                h = Mustache.render(template, { "total": e.data.len, "time": e.data.time });
                $('#uploadSelectDiv').fadeToggle(250, 'swing', function() {
                    $('#uploadSelectDiv').html(h);
                    $('#uploadSelectDiv').fadeToggle(250, 'swing', function() {
                        setTimeout(function() {
