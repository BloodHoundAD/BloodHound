...
export function buildSearchQuery(searchterm) {
    if (searchterm.includes(':')) {
        ...
        let statement = `MATCH (n:${type}) WHERE n.name CONTAINS $name OR n.objectid CONTAINS $name RETURN n ORDER BY size(n.name) LIMIT 10`;

        return [statement, term.toUpperCase()];
    } else {
        return [
            'MATCH (n:Base) WHERE n.name CONTAINS $name OR n.objectid CONTAINS $name RETURN n ORDER BY size(n.name) LIMIT 10',
