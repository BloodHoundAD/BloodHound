...
const NodePlayCypherLink = ({ property, target, baseQuery, distinct, start, end, domain }) => {
    ...
    return (
        <>
            <dt>{property}</dt>
            <dd>
                {!played && <Icon glyph='play' extraClass={styles.icon} onClick={() => { startQuery() }} />}
                {played && <NodeALink ready={ready}
                    value={value}
                    click={() => {
