...
export default class GraphContainer extends Component {
    constructor(props) {
        ...
        child = fork(join(__dirname, "src", "js", "worker.js"), { silent: true });
    ...
    setHighVal(id, status) {
        ...
        q.run(`MATCH (n:${node.type} {${key}:{node}}) SET n.highvalue={status}`, {
            node: keyVal,
            status: status
        }).then(x => {
            q.close();
    ...
    deleteEdge(id) {
        ...
        let q = driver.session();
        q.run(statement, { sname: sourcenode.label, tname: targetnode.label }).then(
            x => {
                q.close();
            }
        );
        ...
        readFile(
            ...
            function(err, data) {
                ...
                try {
                    ...
                    emitter.emit("showAlert", { text: "Bad JSON File", type: "danger" });
                    return;
                }
            ...
            function(_, node) {
                if (node.hasOwnProperty("blocksinheritance")) {
                    if (node.blocksinheritance === true) {
                        let targets = [];
                        $.each(
                            this.state.sigmaInstance.graph.outNeighbors(node.id),
                            function(_, nodeid) {
                        ...
                        $.each(
                            this.state.sigmaInstance.graph.adjacentEdges(node.id),
                            function(_, edge) {
        ...
        return (
            <div className="graph">
                <div
                    id="graph"
                    className={
                        this.state.darkMode ? "graph graph-dark" : "graph graph-light"
                    }
                />
            </div>
        );
    ...
    doQueryNative(params) {
        ...
        if (edgearr.length === 0) {
            emitter.emit("showAlert", {
                text: "Must specify at least one edge type",
                type: "warn"
            });
            emitter.emit("updateLoadingText", "Done!");
            setTimeout(function() {
                        ...
                        if (field !== null) {
                            if (field.hasOwnProperty("segments")) {
                                $.each(
                                    field.segments,
                                    function(_, segment) {
                                        let end = this.createNodeFromRow(segment.end, params);
                                        let start = this.createNodeFromRow(segment.start, params);
                                        let edge = this.createEdgeFromRow(segment.relationship);
            ...
            function(e) {
                ...
                if (document.activeElement === document.body && key === 17) {
                    ...
                    if (mode === 2) {
                        ...
                        emitter.emit("showAlert", {
                            text: "Hiding Node Labels",
                            type: "info"
                        });
                    } else if (mode === 0) {
                        sigmaInstance.settings("labelThreshold", 15);
                        emitter.emit("showAlert", {
                            text: "Default Node Label Threshold",
                            type: "info"
                        });
                    } else {
                        sigmaInstance.settings("labelThreshold", 1);
                        emitter.emit("showAlert", {
                            text: "Always Showing Node Labels",
                            type: "info"
                        });
                    }
